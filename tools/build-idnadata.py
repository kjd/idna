...
def print_optimised_list(d):
    ...
    for i in range(0, len(codepoint_list)):
        ...
        if len(codepoint_range) == 1:
            ...
            set_elements.append("range({},{})".format(hex(codepoint_range[0]), hex(codepoint_range[-1]+1)))
        last_write = i
...
def build_idnadata(version):
    ...
    print "scripts = {"
    scripts = {}
    for line in urllib2.urlopen(SCRIPTS_URL).readlines():
        line = line.strip()
    ...
    print "joining_types = {"
    scripts = {}
    for line in urllib2.urlopen(JOININGTYPES_URL).readlines():
        line = line.strip()
        if not line or line[0] == '#':
            continue
    ...
    idntables_data = urllib2.urlopen(IDNATABLES_URL.format(version=version)).read()
