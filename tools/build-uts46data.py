...
RE_CHAR_RANGE = re.compile(rb"([0-9a-fA-F]{4,6})(?:\.\.([0-9a-fA-F]{4,6}))?$")
STATUSES = {
    b"valid": ("V", False),
    b"ignored": ("I", False),
    b"mapped": ("M", True),
...
def parse_idna_mapping_table(inputstream):
    ...
    for line in inputstream:
        ...
        if mapping:
            replacement = "".join(chr(int(codepoint, 16))
                for codepoint in fields[2].split())
            ranges.append((start, status, replacement))
        else:
            ranges.append((start, status))
    return ranges
